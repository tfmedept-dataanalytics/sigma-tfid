...
export async function signIn(prevState, formData) {
  ...
  if (!email) {
    // Tidak ada satu pun akun aktif → seed belum dijalankan. Bila sudah ada
    // akun lain, tetap pakai pesan generik agar username tidak bisa ditebak.
    const { count } = await supabase.from('profiles').select('id', { count: 'exact', head: true }).eq('active', true);
    return { error: (count || 0) === 0 ? NO_ACCOUNT : GENERIC };
  }
