...
export default async function ViewPage({ params, searchParams }) {
  ...
  const years = await yearsOf(supabase);
  const year = searchParams?.year || allYearsPick(years);
  const qtr = Number(searchParams?.q) || 2;
  const region = searchParams?.region || 'National';
  const { rows, config, allYears, regions } = await getAll(year);
  ...
  return (
    <ViewHost id={id} rows={rows} year={year} qtr={qtr} allYears={allYears}
              regions={regions} region={region} role={role} config={config} {...extra} />
  );
}
