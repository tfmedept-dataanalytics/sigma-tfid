...
import seed from '@/supabase/seed/sigma-seed.json';
...
export async function POST() {
  ...
  seed.indicators.forEach(i => {
    Object.entries(i.years || {}).forEach(([y, r]) => {
      years.push({
        indicator_id: i.id, year: Number(y), target: r.target ?? null,
  ...
  for (const part of chunk(years, 400)) {
    const { error } = await admin.from('indicator_years')
      .upsert(part, { onConflict: 'indicator_id,year', ignoreDuplicates: true });
