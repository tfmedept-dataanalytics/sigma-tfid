...
**Indicator & Year Management** mendukung siklus penuh: menambah indikator baru pada ketiga
kerangka beserta periode tahun pertamanya, mengubah strukturnya, menambah periode tahunan —
per indikator maupun sekaligus untuk seluruh kerangka dengan opsi menyalin target — serta
menghapus indikator dan periode. Seluruhnya lewat Server Action yang memakai sesi pengguna,
sehingga Row Level Security tetap berlaku: hanya System Administrator dan PMO yang dapat
mengubah struktur, dan hanya System Administrator yang dapat menghapus.

Indikator RPI otomatis memperoleh lima baris per tahun (empat region + Akumulasi Regional).
Modul AI tetap berbasis aturan, bukan model bahasa, dan tetap memisahkan Fakta / Interpretasi
/ Keterbatasan.
...
**Dua bahasa.** Bahasa Indonesia adalah default; English tersedia melalui pemilih bahasa
di halaman login dan di top bar. Pilihan disimpan di `localStorage`.

Terjemahan diterapkan pada **lapisan render**: `components/Translate.js` membungkus isi
halaman, menelusuri pohon React, dan melewatkan setiap teks serta atribut `placeholder`,
`title`, `alt`, dan `aria-label` melalui kamus di `lib/i18n.js`. Pendekatan ini dipilih
karena menyisipkan `t()` ke 46 halaman berisiko merusak halaman yang sudah teruji dan pasti
menyisakan bagian yang terlewat; dengan cara ini halaman baru pun ikut terterjemahkan tanpa
perubahan kode. Frasa yang tidak ada di kamus dikembalikan apa adanya, sehingga yang terlewat
tampil dalam Bahasa Indonesia, bukan kosong. Elemen bertanda `data-no-i18n` dilewati seisinya.

Batas yang perlu diketahui: kalimat yang menyisipkan angka atau variabel terpecah menjadi
beberapa potongan teks di pohon React, sehingga tidak dapat dicocokkan sebagai satu frasa
utuh. Label, judul, tombol, header tabel, keadaan kosong, dan paragraf statis sudah
diterjemahkan; paragraf panjang yang memuat angka masih Bahasa Indonesia. Menambahkannya
cukup dengan menambah entri kamus, tanpa perubahan kode.
