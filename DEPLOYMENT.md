...
### Semua pemeriksaan SQL bernilai true, tetapi login tetap ditolak

Penyebabnya hampir pasti kolom token NULL pada `auth.users`.

Akun yang dibuat langsung lewat SQL - termasuk lewat `0004_bootstrap_admin.sql` versi
awal - meninggalkan `confirmation_token`, `recovery_token`, `email_change`, dan kolom
sejenis bernilai NULL. GoTrue, layanan auth Supabase, membaca kolom itu ke tipe string
Go yang tidak menerima NULL, sehingga setiap permintaan login gagal di lapisan auth
meskipun hash password-nya benar. Karena kegagalannya terjadi sebelum pemeriksaan
kredensial, aplikasi hanya menerima penolakan biasa dan menampilkan pesan generik.

Perbaikannya: jalankan `supabase/FIX_LOGIN_NULL_TOKENS.sql`, atau `supabase/FIX_LOGIN.sql`
versi terbaru yang sudah memuat langkah tersebut pada bagian 3b. Query verifikasi di
akhir file harus menghasilkan 0 pada seluruh kolom.

Akun yang dibuat lewat `npm run seed`, `npm run create-admin`, atau halaman `/setup`
tidak pernah mengalami masalah ini, karena ketiganya memakai Auth Admin API yang mengisi
kolom-kolom tersebut dengan benar.

### Build gagal di Vercel padahal berkas sudah diperbarui di GitHub

Penyebab paling sering: **berkas lama masih tertinggal di repository.** Mengunggah
berkas lewat web GitHub menambah dan menimpa, tetapi tidak menghapus berkas yang sudah
tidak dipakai. Berkas lama itu tetap ikut di-build dan menggagalkannya, biasanya dengan
pesan `Module not found: Can't resolve '@/lib/...'`.

Route berikut sudah digantikan satu route dinamis `app/(app)/v/[id]/page.js` dan harus
dihapus dari repository bila masih ada:
...
Langkah rincinya ada di `CLEANUP.md`.

**Membaca pesan errornya.** Pada halaman Build Logs di Vercel, gulir ke bawah melewati
baris `next build`. Baris yang menyebutkan penyebabnya berada di bawah situ, biasanya
diawali `Failed to compile` atau `Module not found`. Baris di bagian atas log hanya
mencatat proses instalasi yang berhasil.

Rilis ini juga menyertakan `lib/trend.js` sebagai shim kompatibilitas, sehingga berkas
lama yang masih mengimpornya tidak lagi menggagalkan build. Shim itu tetap tidak
menghapus keharusan membersihkan route lama.
