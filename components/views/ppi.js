...
export function PpRepo({ rows, year, allYears, role }) {
  return (
    <>
      ...
      <IndicatorTable type="PPI" year={year} years={allYears} role={role} rows={PPI(rows)} />
    </>
  );
}

export function PpQuarterly({ rows, year, allYears, role }) {
  return (
    <>
      <PageHead title="Quarterly Update — Program Performance">
        Input actual per kuartal untuk indikator program. Nilai kosong berarti belum ada data dan
        tidak pernah dihitung sebagai nol.
      </PageHead>
      <IndicatorTable type="PPI" year={year} years={allYears} role={role} rows={PPI(rows)} />
    </>
  );
