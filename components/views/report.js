...
import { Card, Kpi, Note, NoData, PageHead, Badge, BarList, groupBy, statSummary,
         achievement, actualOf, fmt, pct, useState, useMemo } from './common';
...
function ReportBody({ rows, year, scope, sections }) {
  ...
  return (
    <Card right={<button className="btn sm no-print" onClick={() => window.print()}>Cetak / PDF</button>}>
      <div style={{ borderBottom: '2px solid var(--green)', paddingBottom: 10, marginBottom: 16 }}>
        ...
        <h3 style={{ margin: '4px 0 0' }}>{scope} — {year}</h3>
...
export const RpQuarterly = ({ rows, year }) =>
  <><PageHead title="Quarterly Report">Laporan kuartalan untuk periode berjalan.</PageHead>
    <ReportBody rows={rows} year={year} scope="Laporan Kuartalan" sections={ALL} /></>;
