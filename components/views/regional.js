...
export function useRegion(regions = [], regionProp) {
  ...
  const fallback = regions.includes(DEFAULT_REGION) ? DEFAULT_REGION : (regions[0] || DEFAULT_REGION);
...
export function RgQuarterly({ rows, year, qtr, regions = [], region: regionProp, role }) {
  ...
  return (
    <>
      ...
      <div className="grid g4 mb">
        <Kpi label={`Terisi Q${qtr}`} value={f.filled} detail={`region ${region}`} cls="b" />
        <Kpi label={`Belum terisi Q${qtr}`} value={f.total - f.filled} detail="menunggu input" cls="a" />
        <Kpi label="Kelengkapan" value={`${(f.ratio * 100).toFixed(0)}%`} detail={`Q${qtr} ${year} · ${region}`} cls="gd" />
        <Kpi label="Total indikator" value={rpi.length} detail="KPI regional" />
      </div>
      <RegionalTable rows={rpi} year={year} qtr={qtr} regions={regions} role={role}
