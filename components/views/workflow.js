...
import { Card, Kpi, Note, NoData, PageHead, Badge, StatusBadge, WfBadge, Progress, Spark,
         statSummary, PeriodChip, quarterFill, qVal, achievement, actualOf, fmt, pct, useMemo } from './common';
import { can, WORKFLOW } from '@/lib/calc';
import Link from 'next/link';
import { hrefOf } from '@/lib/nav';

function Queue({ title, desc, rows, year, qtr = 2, status, empty, note }) {
  ...
  return (
    <>
      <PageHead title={title} right={<PeriodChip year={year} qtr={qtr} />}>{desc}</PageHead>
        ...
        <Card sub={`${list.length} indikator dalam antrean ini`}>
          <div className="tbl-w"><table>
            <thead><tr><th>ID</th><th style={{ minWidth: 280 }}>Indikator</th>
              ...
              <th className="ctr">Q{qtr}</th><th>Workflow</th><th className="ctr">Buka</th></tr></thead>
            <tbody>
              {list.slice(0, 200).map(r => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td className="ctr">
                      {qVal(y, qtr) === null || qVal(y, qtr) === undefined
                        ? <Badge cls="b-rd">belum</Badge>
                        : <Badge cls="b-ok">{fmt(qVal(y, qtr), r.unit)}</Badge>}
                    </td>
...
export function WfMyTasks({ rows, year, qtr, role }) {
  ...
  return (
    <Queue
      ...
      year={year} qtr={qtr} status={['draft', 'returned']}
...
export function WfReview({ rows, year, qtr }) {
  return (
...
export function WfValidation({ rows, year, qtr }) {
  return (
    <Queue title="Validation"
      ...
      rows={rows} year={year} qtr={qtr} status={['review']}
...
export function WfApproval({ rows, year, qtr }) {
  return (
    <Queue title="Approval"
      ...
      rows={rows} year={year} qtr={qtr} status={['review', 'submitted']}
      empty={{ t: 'Tidak ada data menunggu persetujuan', h: 'Seluruh data pada periode ini sudah diproses.' }} />
  );
}

export function WfReturned({ rows, year, qtr }) {
  return (
    <Queue title="Returned / Revision"
      ...
      rows={rows} year={year} qtr={qtr} status={['returned']}
