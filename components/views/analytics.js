...
export function AnTrend({ rows, year, allYears, regions }) {
  ...
  return (
    <>
      ...
      <ScopeBar scope={scope} regions={regions} />
      <TrendChart years={t.years} opi={t.opi} rpi={t.rpi} ppi={t.ppi} all={t.all}
                  regionLabel={scope.region} />
      {scope.hasRpi && (
        <Note kind="w">
          Seri RPI hanya mencakup 2025–2026 karena data regional memang baru tersedia untuk kedua
          tahun itu, dan pengisian 2025 sangat tipis. Garisnya belum layak dibaca sebagai tren.
        </Note>
      )}
    </>
  );
