...
export function OrgRepo({ rows, year, qtr, allYears, role }) {
  return (
    <>
      ...
      <IndicatorTable type="OPI" year={year} qtr={qtr} years={allYears} role={role} rows={OPI(rows)} />
    </>
  );
}

export function OrgQuarterly({ rows, year, qtr, allYears, role }) {
  const opiRows = OPI(rows);
  const f = quarterFill(opiRows, year, qtr);
  return (
    <>
      <PageHead title="Quarterly Update — Organization Performance"
                right={<PeriodChip year={year} qtr={qtr} />}>
        Input actual untuk Q{qtr} {year}. Kolom kuartal aktif disorot pada tabel. Alur: pemilik
        indikator mengisi nilai, menambahkan commentary dan evidence, lalu submit untuk direview.
        Nilai yang berubah menurunkan status Approved kembali ke Draft.
      </PageHead>
      <div className="grid g4 mb">
        <Kpi label={`Terisi Q${qtr}`} value={f.filled} detail={`dari ${f.total} indikator ber-periode ${year}`} cls="b" />
        <Kpi label={`Belum terisi Q${qtr}`} value={f.total - f.filled} detail="menunggu input pemilik indikator" cls="a" />
        <Kpi label="Kelengkapan" value={`${(f.ratio * 100).toFixed(0)}%`} detail={`pada Q${qtr} ${year}`} cls="gd" />
        <Kpi label="Total indikator" value={opiRows.length} detail="KPI organisasi" />
      </div>
      <IndicatorTable type="OPI" year={year} qtr={qtr} years={allYears} role={role} rows={OPI(rows)} />
    </>
  );
