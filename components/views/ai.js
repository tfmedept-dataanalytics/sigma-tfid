  ...
  return (
    <>
      ...
      <Card>
        <Facts>
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            <li>{s.total} indikator dalam cakupan{scope.type ? ` (${scope.type})` : ''}; {s.withA} memiliki
                target dan actual sekaligus pada {year}.</li>
            ...
            <li>OPI: {pct(sOpi.score)} dari {sOpi.withA} indikator ber-data.
                RPI: {pct(sRpi.score)} dari {sRpi.withA} ({scope.region}).
...
export function AiNarrative({ rows, year, regions }) {
  ...
  const sRpi = statSummary(list.filter(r => r.type === 'RPI'), year);
  const text =
`Pada ${year}, cakupan yang dibaca memuat ${s.total} indikator kinerja: ${sOpi.total} indikator organisasi (OPI), ${sRpi.total} indikator regional (RPI, dibaca pada ${scope.region}), dan ${sPpi.total} indikator program (PPI). Dari jumlah tersebut, ${s.withA} indikator memiliki target dan actual sekaligus sehingga capaiannya dapat dihitung.

Rata-rata capaian gabungan tercatat ${pct(s.score)}, dihitung secara tidak berbobot dari ${s.withA} indikator yang datanya lengkap. Pada kerangka organisasi, rata-rata capaian ${pct(sOpi.score)} berasal dari ${sOpi.withA} indikator; kerangka regional ${pct(sRpi.score)} dari ${sRpi.withA} indikator; kerangka program ${pct(sPpi.score)} dari ${sPpi.withA} indikator.
