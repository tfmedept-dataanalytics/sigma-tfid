...
export default function IndicatorTable({ type, year, qtr = 2, years, role, rows: initial }) {
  const { t } = useLang();
    ...
    <>
      ...
      <div className="card mb"><div className="card-b" style={{ padding: '12px 14px' }}>
        <div className="filters">
          ...
          <select value={qF} onChange={e => setQF(e.target.value)} title={`Kelengkapan Q${qtr}`}>
            <option value="">Semua kelengkapan Q{qtr}</option>
      ...
      <div className="card"><div className="tbl-w"><table>
        <thead><tr>
          ...
          <th>{t('Status')}</th><th>{t('Workflow')}</th>
        </tr></thead>
        <tbody>
          {view.map(r => {
            const a = achievement(r.year, r);
            const cls = statusClass(a);
            const wf = WORKFLOW[r.year.status] || WORKFLOW.draft;
            return (
              <tr key={r.id}>
                ...
                <td className="num">
                  {editable
                    ? <input className="qin" key={`t-${r.id}-${year}`} defaultValue={toInput(r.year.target, r.unit)}
                             onBlur={e => saveCell(r.id, 'target', e.target.value)}
                             disabled={!!saving[r.id + 'target']} />
                    : fmt(r.year.target, r.unit)}
                </td>
                {['q1', 'q2', 'q3', 'q4'].map((k, ki) => (
                  <td className="ctr" key={k}
                      style={ki + 1 === qtr ? { background: 'var(--green-x)' } : undefined}>
                    {editable
                      ? <input className="qin" key={`${k}-${r.id}-${year}`} defaultValue={toInput(r.year[k], r.unit)}
