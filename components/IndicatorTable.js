...
export default function IndicatorTable({ type, year, years, role, rows: initial }) {
  const { t } = useLang();
          ...
          {view.map(r => {
            ...
            return (
              <tr key={r.id}>
                ...
                <td className="num">
                  {editable
                    ? <input className="qin" defaultValue={toInput(r.year.target, r.unit)}
                ...
                {['q1', 'q2', 'q3', 'q4'].map(k => (
                  <td className="ctr" key={k}>
                    {editable
                      ? <input className="qin" defaultValue={toInput(r.year[k], r.unit)}
