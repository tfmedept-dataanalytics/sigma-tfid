...
export default function Translate({ children }) {
  ...
  /* Penelusuran pohon hanya sah untuk elemen yang dibuat di sisi klien.
     Bila komponen ini dipasang di Server Component, children-nya berupa
     muatan RSC yang tidak dapat di-clone di browser — karena itu kegagalan
     apa pun dikembalikan sebagai teks asli, bukan menjatuhkan halaman. */
  try {
    return walk(children, t);
  } catch (e) {
    console.error('[SIGMA] terjemahan dilewati:', e?.message || e);
    return children;
  }
}
