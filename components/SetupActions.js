  ...
  async function call(what) {
    ...
    try {
      const res = await fetch('/api/setup/' + what, {
        ...
        body: JSON.stringify(what === 'bootstrap' && pw.trim() ? { password: pw.trim() } : {})
      });
  ...
  return (
    <div className="card mb">
      ...
      <div className="card-b">
        ...
        {!hasAdmin && (
          <div className="fld">
            <label htmlFor="bootpw">Password administrator (opsional)</label>
            <input id="bootpw" value={pw} onChange={e => setPw(e.target.value)}
                   placeholder="kosongkan untuk dibuatkan otomatis" autoComplete="off" />
            <div className="hint">
              Minimal 8 karakter. Bila diisi, password inilah yang dipakai — sehingga Anda tidak
              bergantung pada teks yang hanya muncul sekali. Wajib diganti saat pertama masuk.
            </div>
          </div>
        )}
