...
const DEF = {
  ...
  /* RPI memakai hierarki yang sama dengan OPI — keduanya berpijak pada
     Balanced Scorecard yang sama; yang membedakan hanya dimensi region. */
  RPI: [
    { k: 'id', l: 'KPI Regional' },
    { k: 'accountability', l: 'Accountability' },
    { k: 'outcome', l: 'Outcome' },
    { k: 'strategy_map', l: 'Strategy Map' }
  ]
};
...
export default function Pathway({ rows, year, regions = [] }) {
  const [type, setType] = useState('PPI');
  const [region, setRegion] = useState(ACCUM);
  ...
  const def = DEF[type];
  /* Untuk RPI, baris tahun aktif dipetakan ke region terpilih. Tanpa ini
     seluruh jalur akan memakai baris Akumulasi Regional dan mengganti region
     tidak mengubah angka pada garis penghubung. */
  const base = useMemo(() => rows.filter(r => r.type === type).map(r => {
    if (type !== 'RPI') return r;
    const row = r.regions?.[year]?.[region] || {};
    return { ...r, year: row, years: { ...r.years, [year]: row } };
  }), [rows, type, year, region]);
  ...
  return (
    <>
      ...
      {!list.length ? (
        ...
        <div className="card mb">
          <div className="card-h">
            <h3>Pathway {type} — tahun {year}{type === 'RPI' ? ` · ${region}` : ''}</h3>
            <div className="sub">
      ...
      <div className="note w">
        ...
        {type === 'RPI' && (
          <> Pada kerangka RPI, angka pada garis dibaca untuk region <b>{region}</b>; jalur strukturnya
          sendiri sama untuk seluruh region karena hierarki Balanced Scorecard tidak berbeda antar region.</>
        )}
      </div>
    </>
  );
