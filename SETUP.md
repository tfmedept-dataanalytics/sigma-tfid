...
| 7 | `supabase/migrations/0008_rpi_region.sql` |
| 8 | `supabase/migrations/0009_regions_four.sql` |
...
1. `supabase/seed/seed_1a_indicators_opi.sql` — 80 indikator OPI
2. `supabase/seed/seed_1b_indicators_ppi_1.sql` — 160 indikator PPI
3. `supabase/seed/seed_1c_indicators_ppi_2.sql` — 147 indikator PPI
4. `supabase/seed/seed_2_values.sql` — 461 baris nilai per tahun
5. `supabase/seed/seed_3_rpi.sql` — 23 indikator RPI dan 135 baris nilai per region
...
Harus menghasilkan 410 / 596 / 1 setelah seed RPI ikut dimuat.

`seed_3_rpi.sql` mengganti seluruh data RPI: ia menghapus indikator bertipe RPI
lebih dulu agar indikator yang sudah tidak ada pada file sumber tidak tertinggal.
Data OPI dan PPI tidak tersentuh.
...
Muncul bila `seed_2_values.sql` versi lama dijalankan setelah migrasi 0008. Migrasi itu
mengubah primary key `indicator_years` dari `(indicator_id, year)` menjadi
`(indicator_id, year, region)`, sehingga klausa `on conflict (indicator_id, year)` pada
berkas seed lama tidak lagi cocok dengan constraint mana pun.

Pakai `seed_2_values.sql` dan `seed_3_rpi.sql` versi terbaru. Keduanya memakai
`on conflict (indicator_id, year, region)` dan diawali blok yang memastikan kolom serta
primary key sudah sesuai, sehingga urutan menjalankannya tidak lagi menentukan.

### Error `relation "the" does not exist` saat menjalankan seed indikator

Beberapa teks indikator memuat tanda titik koma di tengah kalimat. Bila berkas dipotong
manual atau editor memisah perintah berdasarkan titik koma, potongannya terputus di tengah
literal string dan Postgres membaca sisa kalimat sebagai perintah SQL — sehingga muncul
pesan seperti `relation "the" does not exist`.

Berkas seed terbaru sudah menghilangkan seluruh titik koma dari dalam literal: karakter itu
ditulis sebagai `chr(59)` dan disambung dengan tanda pipa ganda, sehingga nilainya tetap
sama persis di database. `seed_1` juga dipecah menjadi tiga berkas berukuran 28–93 KB agar
tidak perlu dipotong manual.
